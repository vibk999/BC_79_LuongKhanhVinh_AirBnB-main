...
export default function TicketByUser() {
  ...
  const fectchRoomTicket = async () => {
    if (stateUser.userInfo) {
      ...
      const roomTicketData = await roomService.fetchRoomTicketApi(
        stateUser.userInfo.user.id
      );
      const roomTickets = roomTicketData.data.content;
      const roomDetails = [];
      for (const room of roomTickets) {
        const roomTicketDetail = await roomService.fetchRoomDetailApi(
          room.maPhong
        );
            ...
            <div className="flex flex-col mt-4 md:pr-5 lg:w-2/3">
              {roomTicket.map((ticket, index) => (
                ...
                >
                  <div className="w-1/4">
                    <img
                      src={roomTicketDetail[index].hinhAnh}
                      alt={roomTicketDetail[index].tenPhong}
                  ...
                  <div className="w-3/4 px-4 py-2">
                    <div className="flex justify-between items-center md:border-b pb-2">
                      <div className="text-center md:text-left">
                        <h3 className="font-semibold text-xl md:text-xl uppercase">
                          {roomTicketDetail[index].tenPhong}
                        </h3>
                      ...
                      <div>
                        <span className="font-semibold text-sm md:text-xl">
                          ${roomTicketDetail[index].giaTien}
